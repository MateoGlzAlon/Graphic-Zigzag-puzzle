    ...
    public class ZigZag {
    	...
    	private static String[][] tablaAmpliada;
    	
    	private int intentos;
    
    	    
    	
    	public ZigZag(int filas, int columnas, String[][] tablaAmpliada) {
    		ZigZag.filas = filas;
    		ZigZag.columnas = columnas;
    		ZigZag.tablaAmpliada = tablaAmpliada;
    	}    	
    	...
    	public String solucionar() {
        	...
        	//Si la entrada está mal formada, atrapa la excepcion y muestra Entrada incorrecta.
				...
				aux += "Entrada incorrecta por malformacion";
				return aux;
			}
    		
    		//Si un elemento es menor de 1 o mayor de 10, la entrada es incorrecta
    		if(elementoIncorrecto(tablaAmpliada)) {
    			aux += "Entrada incorrecta ya que un numero es mayor de 10 o menor que 10.";
				return aux;
    		}
    		...
    		for (int j = 0; j< soluciones.size();j++) {
    			...
    			aux += "Solucion nº: " + (j+1) + "\n";
    			aux += mostrarTabla2D(tablaAmpliada);
    			limpiarTabla(tablaAmpliada);
...
//    		    		System.out.println();
    	...
    	public static String mostrarTabla2D(String[][] tabla) {
    		String aux = "";
    		
    		for (int i = 0; i < filas*2-1; i++) {
    			for (int j = 0; j < columnas*2-1; j++) {
    				if(esSimbolo(tabla[i][j])) {
    					aux += " " + tabla[i][j] + " ";
    				} else {
        				aux += tabla[i][j] + "";
    				}
    	...
    	public static boolean esSimbolo(String aux) {
    	switch (aux) {
